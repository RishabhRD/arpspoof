 ...
 * Returns 0 on success.
 */
int test_arping(const char *ifname, const char *ip,const char *ip_name) {
	int ret = -1;
...
int main(int argc, const char **argv) {
	int ret = -1;
	if (argc != 4) {
		printf("Usage: %s <INTERFACE> <SRC_IP> <DEST-IP>\n", argv[0]);
		return 1;
	}
	const char *ifname = argv[1];
	const char *ip = argv[3];
	const char *ip_name = argv[2];
	return test_arping(ifname, ip,ip_name);
}
